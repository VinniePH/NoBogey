...
export function CaddieListingScreen() {
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safeArea}>
    ...
    <CaddieDetailSheet caddie={caddies.find((caddie) => caddie.id === selectedId) ?? null} course={courses.find((course) => course.id === (courseId ?? caddies.find((caddie) => caddie.id === selectedId)?.homeCourseId))} onBook={() => isGlobalDirectory ? router.push({ pathname: "/courses", params: { caddieId: selectedId, courseId: caddies.find((caddie) => caddie.id === selectedId)?.homeCourseId } }) : router.push({ pathname: "/booking", params: { caddieId: selectedId, courseId, date, teeTimeId, time } })} onClose={() => setSelectedId(undefined)} visible={Boolean(selectedId)} />
  </SafeAreaView>;
}
