...
import { TourTarget, useAutomaticGuidedTour, useGuidedTour } from "../guided-tour/GuidedTour";
...
export function CaddieDashboardScreen() {
  useAutomaticGuidedTour("caddie");
  const { activeTarget, refreshTarget } = useGuidedTour();
  ...
  useEffect(() => {
    ...
    if (activeTarget === "caddie-tab-schedule") setTab("schedule");
    if (activeTarget === "caddie-tab-roster") setTab("roster");
    if (activeTarget === "caddie-tab-portfolio") setTab("portfolio");
    const y = activeTarget.startsWith("caddie-tab-") ? Math.max(0, tabsY - spacing.xl) : 0;
    const frame = requestAnimationFrame(() => scrollRef.current?.scrollTo({ animated: true, y }));
    const timeout = setTimeout(refreshTarget, 350);
    return () => { cancelAnimationFrame(frame); clearTimeout(timeout); };
  }, [activeTarget, refreshTarget, tabsY]);
  const focusRoster = () => {
    setTab("roster");
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safeArea}><ScrollView ref={scrollRef} contentContainerStyle={styles.scrollContent} contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false}><ResponsiveContent style={styles.content}>
    <TourTarget id="caddie-dashboard"><DashboardHeader notificationCount={getUnreadCount("caddie")} onNotifications={focusRoster} /></TourTarget>
    ...
    <Metrics />
    <View accessibilityRole="tablist" onLayout={(event) => setTabsY(event.nativeEvent.layout.y)} style={styles.tabs}><TourTarget id="caddie-tab-schedule" style={styles.tabTarget}><Tab active={tab === "schedule"} label="Schedule" onPress={() => setTab("schedule")} /></TourTarget><TourTarget id="caddie-tab-roster" style={styles.tabTarget}><Tab active={tab === "roster"} label="Roster" onPress={() => setTab("roster")} /></TourTarget><TourTarget id="caddie-tab-portfolio" style={styles.tabTarget}><Tab active={tab === "portfolio"} label="Portfolio" onPress={() => setTab("portfolio")} /></TourTarget></View>
