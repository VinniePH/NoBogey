...
// TODO: load availability slots from the real caddie availability service.
...
export function CaddieAvailabilityEditor({ onClose, onSave, slots }: { onClose: () => void; onSave: (slots: AvailabilitySlot[]) => void; slots: AvailabilitySlot[] }) {
  ...
  return <Modal animationType="slide" transparent visible onRequestClose={onClose}>
    <View style={styles.backdrop}><View accessibilityViewIsModal style={styles.modalCard}>
      ...
      {slots.length ? <View style={styles.calendar}><CalendarGrid editable onSlotPress={updateSlot} slots={slots} /></View> : <EmptyState description="Availability can be edited after the schedule service supplies time slots." icon="calendar-blank-outline" title="No availability data" />}
