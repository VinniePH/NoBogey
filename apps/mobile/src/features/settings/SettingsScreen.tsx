...
function getSections(role: SettingsRole, actions: { openSheet: (sheet: SettingsSheet) => void; openTerms: () => void }): { title: string; items: SettingsItem[] }[] {
...
function NotificationContent({ notifications, setNotifications }: { notifications: { alerts: boolean; promotions: boolean }; setNotifications: Dispatch<SetStateAction<{ alerts: boolean; promotions: boolean }>> }) { return <><Text style={styles.sheetCopy}>These local switches affect this device session only.</Text>{([['alerts', 'Booking or match alerts'], ['promotions', 'Promotions']] as const).map(([key, label]) => <View key={key} style={styles.toggleRow}><Text style={styles.infoLabel}>{label}</Text><Switch accessibilityLabel={label} accessibilityRole="switch" onValueChange={(value) => setNotifications((current) => ({ ...current, [key]: value }))} trackColor={{ false: colors.border, true: colors.primary }} value={notifications[key]} /></View>)}</>; }
