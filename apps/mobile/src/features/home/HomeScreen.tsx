...
export function HomeScreen() {
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safeArea}><ScrollView contentContainerStyle={styles.content} contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false}><ResponsiveContent style={{ gap: 44 }}>
    ...
    <HomeSection actionLabel="See All" onAction={() => router.push("/all-courses")} title="Courses"><ScrollView horizontal contentContainerStyle={styles.horizontalList} showsHorizontalScrollIndicator={false}>{courses.map((course) => <CourseCard compact course={course} key={course.id} onPress={() => router.push({ pathname: "/courses/[id]", params: { id: course.id } })} />)}</ScrollView></HomeSection>
    <HomeSection actionLabel="See All" onAction={() => router.push("/all-caddies")} title="Caddies"><ScrollView horizontal contentContainerStyle={styles.horizontalList} showsHorizontalScrollIndicator={false}>{caddies.map((caddie) => <CaddieCard caddie={caddie} compact key={caddie.id} onPress={() => setSelectedCaddieId(caddie.id)} />)}</ScrollView></HomeSection>
  </ResponsiveContent></ScrollView><BottomNavigation /><CaddieDetailSheet caddie={selectedCaddie} course={courses.find((course) => course.id === selectedCaddie?.homeCourseId)} onBook={() => router.push({ pathname: "/courses", params: { caddieId: selectedCaddie?.id, courseId: selectedCaddie?.homeCourseId } })} onClose={() => setSelectedCaddieId(undefined)} visible={Boolean(selectedCaddie)} /></SafeAreaView>;
}
...
function BottomNavigation() { return <View accessibilityRole="tablist" style={styles.bottomNavigation}>
  ...
  <NavTab icon="calendar" label="Bookings" onPress={() => router.push("/bookings")} />
  <Pressable accessibilityLabel="Find a Game" accessibilityRole="button" onPress={() => router.push("/find-game")} style={styles.primaryNavAction}><GolfBallIcon /></Pressable>
